...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    const rings: Ring[] = [
      ...
      { rotX: 0.3, rotY: 0.7, rotZ: 1.1, vX: -0.004, vY: 0.003, vZ: -0.002, radius: 160, segments: 12 },
    ]
    ...
    function drawRing(ring: Ring, cx: number, cy: number) {
      ...
      for (let i = 0; i < ring.segments; i++) {
        ...
        const rx = Math.cos(angle) * ring.radius
        const ry = Math.sin(angle) * ring.radius
        const rz = 0
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        c.strokeStyle = `rgba(249,8,8,${alpha})`
        c.lineWidth = 1
        c.beginPath()
        ...
        c.stroke()
        if (i % 3 === 0) {
          const ptCC = pts[(i + Math.floor(ring.segments / 3)) % pts.length]
          c.strokeStyle = `rgba(249,8,8,${alpha * 0.4})`
          c.beginPath()
          c.moveTo(ptA.x, ptA.y)
          c.lineTo(ptCC.x, ptCC.y)
          c.stroke()
        }
      }
      ring.rotX += ring.vX
    ...
    function animate() {
      ...
      g.addColorStop(0, 'rgba(249,8,8,0.04)')
      g.addColorStop(1, 'rgba(249,8,8,0)')
      c.fillStyle = g
      c.fillRect(0, 0, W, H)

      const mg = c.createRadialGradient(mouse.x, mouse.y, 0, mouse.x, mouse.y, 160)
      mg.addColorStop(0, 'rgba(249,8,8,0.05)')
      mg.addColorStop(1, 'rgba(249,8,8,0)')
      c.fillStyle = mg
      c.fillRect(0, 0, W, H)
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (d < 90) {
            c.strokeStyle = `rgba(249,8,8,${0.06 * (1 - d / 90)})`
            c.lineWidth = 0.5
            c.beginPath()
  ...
  return (
    <section ref={sectionRef} style={{ position:'relative', height:'100vh', minHeight:'700px', display:'flex', alignItems:'center', overflow:'hidden', background:'#ffffff', willChange:'transform, opacity' }}>
      ...
      <div style={{ position:'relative', zIndex:2, maxWidth:'1200px', margin:'0 auto', padding:'0 3rem', width:'100%' }}>
        <div style={{ maxWidth:'640px' }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:'0.6rem', marginBottom:'2rem', padding:'0.45rem 1.1rem', border:'1px solid rgba(249,8,8,0.25)', color:'var(--red)', fontFamily:'Inter,sans-serif', fontSize:'0.72rem', letterSpacing:'0.16em', textTransform:'uppercase', background:'rgba(249,8,8,0.03)' }}>
          ...
          <h1 style={{ fontFamily:'Sora,sans-serif', fontWeight:800, fontSize:'clamp(2.8rem,5.5vw,5.2rem)', lineHeight:1.03, color:'var(--dark)', marginBottom:'1.5rem', letterSpacing:'-0.03em' }}>
            Simplifying<br />Digital.<br /><span style={{ color:'var(--red)' }}>Amplifying Growth.</span>
          </h1>
          <p style={{ fontFamily:'Inter,sans-serif', fontWeight:300, fontSize:'clamp(1rem,1.8vw,1.15rem)', color:'var(--muted)', marginBottom:'2.5rem', lineHeight:1.85, maxWidth:'500px' }}>
            NIXRIX creates digital foundations that help businesses look sharper, work smarter, and scale with confidence.
          </p>
          <div style={{ display:'flex', gap:'1rem', flexWrap:'wrap' }}>
            <Link href="/contact"
              style={{ background:'var(--red)', color:'white', padding:'1rem 2.2rem', fontFamily:'Inter,sans-serif', fontWeight:600, fontSize:'0.9rem', display:'inline-flex', alignItems:'center', gap:'0.6rem', transition:'background 0.2s', borderRadius:'50px', boxShadow:'0 4px 20px rgba(249,8,8,0.3)', textDecoration:'none' }}
            ...
            <Link href="/work"
              style={{ background:'transparent', color:'var(--dark)', padding:'1rem 2.2rem', fontFamily:'Inter,sans-serif', fontWeight:600, fontSize:'0.9rem', border:'2px solid var(--dark)', display:'inline-flex', alignItems:'center', gap:'0.6rem', transition:'all 0.2s', borderRadius:'50px', textDecoration:'none' }}
